...
import yesserTest.TestTextToSpeech.TextToSpeech;
...
public class articleCardClientController implements Initializable {
    ...
    public void initializeData(Article article) throws IOException {
        TextToSpeech textToSpeech1 = new TextToSpeech();
        try {
            url = textToSpeech1.T(article.getContenu_art());
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
        }

        textToSpeech.setOnMouseClicked(mouseEvent -> {
            linkText.setText(url);
            Path destination = Paths.get(url);
            try {
