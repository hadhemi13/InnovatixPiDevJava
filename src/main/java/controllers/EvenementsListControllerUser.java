...
public class EvenementsListControllerUser implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        if (EvenementsListControllerUser.getCategoryModelShow() == 0) {
            categoriesModel.setVisible(false);
        } else if (EvenementsListControllerUser.getCategoryModelShow() == 1) {
    ...
    public static void setCategoryModelShow(int categoryModelShow) {
        EvenementsListControllerUser.categoryModelShow = categoryModelShow;
    }
    ...
    private void open_addEvenement(MouseEvent event) throws IOException {
        ...
        showNotification("ç'est interdit !!!", "tu n'as pas accès pour faire ajout de evenement!!!.", NotificationType.ERROR);
    ...
    void close_CategoriesModel(MouseEvent event) {
        categoriesModel.setVisible(false);
        EvenementsListControllerUser.setCategoryModelShow(0);
    }
    ...
    private void setEvenementGridPaneList() {
        ...
        try {
            for (Evenement evenement : evenements) {
                FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/FXML/project/OneEvenementListCardUser.fxml"));
                HBox oneEvenementCard = fxmlLoader.load();
                OneEvenementListCardControllerUser evenementCardControllerUser = fxmlLoader.getController();
                evenementCardControllerUser.setEvenementData(evenement);
                if (column == 1) {
                    column = 0;
                    ++row;
                }
